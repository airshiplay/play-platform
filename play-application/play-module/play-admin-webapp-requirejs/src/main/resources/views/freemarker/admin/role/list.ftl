			...
			function(domReady) {
				domReady(function() {
					...
					var grid = $("#grid-app")
							.bootgrid(
								...
								<div class=\"btn-group btn-group-sm\" data-row-id=\""+row.id+"\">\
									...
									<a type=\"button\" class=\"btn btn-primary \" href=\"#page/center/role/"+row.id+"/user/list\" >\
										<i class=\"glyphicon glyphicon-user\"></i>\
									</a>\
									<a type=\"button\" class=\"btn btn-primary\" href=\"#page/center/role/"+row.id+"/menu/edit\">\
					...
					grid.on("loaded.rs.jquery.bootgrid", function() {
						...
						/* $table.find(".command-userList").on("click", function(e) {
							var role_id = $(this).closest(".btn-group").data("row-id");
							
							var roleContentWrapper =$(".role-content-wrapper");
							console.log("11");
							require([ "text!" + base +"/center/role/user/list/"+role_id+".page?roleId="+role_id+"&_t=" + (new Date()).getTime() ], function(html) {
								roleContentWrapper.html(html);
