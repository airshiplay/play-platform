<section class="content-header">
	...
	<ol class="breadcrumb">
		...
		<li class="active"><a href="#/page/book">
				<i class="fa fa-book"></i>我的书籍
			</a></li>
	</ol>
</section>
		...
		<div class="col-xs-12">
			<div class="mailbox-controls">
				<div class="btn-group btn-group-sm">
					<a type="button" class="btn btn-primary" href="#/page/book/add">
			...
			<div class="box box-primary">
				<div class="box-body no-padding">
					<table id="grid-book" class="table table-hover">
						<thead>
							<tr>
								<th data-column-id="id" data-type="numeric" data-identifier="true" data-visible="false">ID</th>
								<th data-column-id="title">标题</th>
								<th data-column-id="kind" data-css-class="hidden-xs" data-header-css-class="hidden-xs">种类</th>
								<th data-column-id="version" data-css-class="hidden-xs" data-header-css-class="hidden-xs">版本</th>
								<th data-column-id="author" data-css-class="hidden-xs" data-header-css-class="hidden-xs">作者</th>
								<th data-formatter="commands" data-css-class="command-column" data-width="80px" data-sortable="false" data-align="center"
			...
			function(domReady) {
				domReady(function() {
					...
					var grid = $("#grid-book")
							.bootgrid(
									{
										url : base + "/center/book/page",
										formatters : {
											"commands" : function(column, row) {
												return "\
								<div class=\"btn-group btn-group-sm\" data-row-id=\""+row.id+"\">\
									<a type=\"button\" class=\"btn btn-primary\" href=\"#/page/book/edit/"+row.id+"\">\
					...
					var installToDevice = function(grid, ids) {
						$.confirm({
							...
							confirm : function() {
								...
								if (selectedRows && selectedRows.length > 0) {
									$.post(base + "/mdmcommand/installBook", {
										bookIds : ids.join(),
					...
					var removeToDevice = function(grid, ids) {
						$.confirm({
							...
							confirm : function() {
								...
								var selectedRows = $table.bootgrid("getSelectedRows");
								if (selectedRows && selectedRows.length > 0) {
									$.post(base + "/mdmcommand/removeBook", {
										bookIds : ids.join(),
					...
					grid.on("loaded.rs.jquery.bootgrid", function() {
						...
						$table.find(".command-installAppLock").on("click", function() {
							var book_id = $(this).closest(".btn-group").data("row-id");

							installToDevice($table, [ book_id ]);
						});

						$table.find(".command-removeAppLock").on("click", function() {
							var book_id = $(this).closest(".btn-group").data("row-id");

							removeToDevice($table, [ book_id ]);
						});

						$table.find(".command-delete").on("click", function(e) {
							var book_id = $(this).closest(".btn-group").data("row-id");

							deleteRow($table, [ book_id ]);
						});
					});
