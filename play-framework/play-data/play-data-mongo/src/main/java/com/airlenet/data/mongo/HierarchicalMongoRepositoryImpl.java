...
import com.airlenet.data.domain.SortNoComparator;
import com.airlenet.data.domain.Tree;
import com.airlenet.data.domain.TreeHelper;
	...
	public <S extends T> S save(S entity) {
		...
		if (!Objects.equal(beforeModifiedTreePath, afterModifiedTreePath)) {
			...
			if (!Strings.isNullOrEmpty(beforeModifiedTreePath)
				...
				Optional<T> sourceParent = findById(sourceParentId);
				List<T> sourceChildren = sourceParent.get().getChildren();
				if (sourceChildren != null) {
					sourceChildren.remove(entity);
					save(sourceParent.get());
				}
			}
		}
